...
export default function ItemRequestsPage() {
  ...
  const fetchData = async (page = 1, status: string | null = null) => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:5000/api/request${query}`,
      );
      const { data, totalPages: fetchedTotalPages } = response.data;

      const formattedRequests = data.map((request: APIResponseRequest) => ({
  ...
  return (
    <div className="mx-auto max-w-6xl p-6">
      <h1 className="mb-4 text-2xl font-bold">Item Requests</h1>
